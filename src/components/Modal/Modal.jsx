...
export function Modal({ setIsOpen, isOpen, clickedMovie }) {
        ...
        <div className={`${isOpen ? "flex" : "hidden"} fixed inset-0 items-center justify-center bg-black/80 z-50`}>
            <div className="bg-zinc-600 rounded-xl shadow-lg p-4 w-full m-2 md:w-1/2">
                <div className="flex flex-col md:flex-row justify-center items-start gap-2">
                    <div className="">
                        <img className="w-36 md:max-w-7xl rounded-2xl" src={`https://image.tmdb.org/t/p/original${clickedMovie.poster_path}`} alt={clickedMovie.name || clickedMovie.original_title} />
                    </div>

                    <div className="text-gray-200">
                        <div>
                            <h1 className="font-bold text-3xl md:text-4xl">{clickedMovie.name || clickedMovie.original_title}</h1>
                        ...
                        <div className="my-4">
                            ...
                            <span className="text-lg font-medium text-yellow-500">{clickedMovie.popularity}</span>
                        </div>
                    </div>
                </div>
